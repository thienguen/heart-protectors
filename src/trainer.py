...
class ModelTrainer:
    ...
    def load_and_preprocess_data(self):
        ...
        self.feature_names = [
            col for col in stats["columns"] if col != self.config["target_column"]
        ]
    ...
    def train_models(self):
        ...
        if "svm" in self.config["models_to_train"]:
            ...
            if self.config.get("perform_hyperparameter_tuning", False):
                ...
                svm_model.hyperparameter_tuning(
                    self.X_train, self.y_train, param_grid=param_grid
                )
        ...
        for model_name, model in self.models.items():
            ...
            if self.config.get("save_predictions", False):
                ...
                pred_path = os.path.join(
                    self.config["results_dir"], f"{model_name}_predictions.csv"
                )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--models",
        type=str,
        default="random_forest,knn,svm,pca",
    ...
    parser.add_argument(
        "--save_predictions", action="store_true", help="Save model predictions"
    )
