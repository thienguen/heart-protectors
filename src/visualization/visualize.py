...
class Visualizer:
    ...
    def load_results(self):
        ...
        try:
            ...
            print(f"Loaded results for {len(self.models)} models: {', '.join(self.models)}")
            return True
    ...
    def plot_confusion_matrices(self, save_path=None):
        ...
        models_with_cm = [model for model, results in self.results.items()
    ...
    def plot_roc_curves(self, save_path=None):
        ...
        for model, pred_df in self.predictions.items():
            if 'true_label' in pred_df.columns and 'probability' in pred_df.columns:
                try:
                    # Calculate ROC curve
                    fpr, tpr, _ = roc_curve(pred_df['true_label'], pred_df['probability'])
                    roc_auc = auc(fpr, tpr)
    ...
    def plot_feature_importance(self, feature_importance_path=None, save_path=None):
        ...
        if feature_importance_path is None:
            feature_importance_path = os.path.join(self.results_dir, 'feature_importance.json')
    ...
    def _plot_model_comparison_on_axis(self, ax):
        ...
        if not comparison_data:
            ax.text(0.5, 0.5, "No metrics data found for comparison",
                  ha='center', va='center', fontsize=12)
            return
    ...
    def _plot_roc_curves_on_axis(self, ax):
        ...
        if not self.predictions:
            ax.text(0.5, 0.5, "No predictions data available",
                  ha='center', va='center', fontsize=12)
            return
    ...
    def _plot_feature_importance_on_axis(self, ax):
        ...
        try:
            ...
            if isinstance(feature_importance, dict):
                ...
                ax.text(0.5, 0.5, "Invalid feature importance format",
                      ha='center', va='center', fontsize=12)
                return
            ...
            # Create bar chart
            sns.barplot(x='Importance', y='Feature', data=df.head(10), palette='viridis', ax=ax)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Visualize Heart Failure Prediction Results')
